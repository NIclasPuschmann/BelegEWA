...
const app = new Vue({
    ...
    data: {
		...
		ip: "iws107.informatik.htw-dresden.de" // IP des HOST Rechners
    },
    methods: {
		fetchData(){
			//fetch("https://"+this.ip+"../datenadm/restshop/api/buecher") // Api-Request
		...
		fetchTransactions(){
			...
			fetch("../datenadm/restshop/api/transactions") // Api-Request
			.then(response => response.json())
			.then((data) => {
	...
	computed: {
		filteredBooks(){ // Suchfilter mit zwei Eingabefeldern search und search2
			return this.books.filter((book) => {
				return (book.Produktitel.match(new RegExp(this.search, 'i')) /* ||
				book.isbn.match(new RegExp(this.search, 'i')) ||
				book.language.match(new RegExp(this.search, 'i')) */  )
				&& (book.PreisBrutto <= parseFloat(this.search2)); 
			});
		},
